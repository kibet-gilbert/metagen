...
def validateInputSamplesheet(sample_id, sr1, sr2, lr) {
    // println "Validating sample_id:${sample_id}, SR1:${sr1}, SR2:${sr2}, LR:${lr}"
...
ch_samplesheet = Channel
    ...
    .splitCsv( header: ['sample_id', 'sr1', 'sr2', 'lr'], skip: 1 )
    .map { row ->
        // println "Row content: ${row}"
        validateInputSamplesheet(row.sample_id, row.sr1, row.sr2, row.lr) 
    }
    .flatMap { sampleEntry ->
        def (sample_id, sr1, sr2, lr) = sampleEntry 
